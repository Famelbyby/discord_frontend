...
export default function FriendsHeader({selectedItem, chooseItem}: HeaderProps) {
    return (
        <div className="friends-header">
            {items.map((item: HeaderItem) => {
              return (
                <Link href={'/friends' + item.linkTo} key={item.title}>
                    <div className={"friends-header-item" + (selectedItem === item.title ? "friends-header-item_selected" : '')} onClick={() => chooseItem(item.title)}>
                        <Image width={16} height={16} className="friends-header-item__img" src={item.imageSrc} alt=''/>
                        {item.title}
                    </div>
                </Link>
              );
